...
> **IMPORTANTE**: 
> 1. Todos los archivos `.feature` DEBEN crearse en la ruta `src/test/java/com/pereira/features/[nombre_microservicio]/`.
...
3. **Formato de escenarios**:
   ...
   **Nota sobre escenarios obligatorios**: Por defecto, todo feature debe incluir al menos estos escenarios:
   - Escenario exitoso (status 200/201)
      - Escenario exitoso con respuesta vacía (status 200/201)
   - Escenario de error de validación (status 400)
    - Escenario de error no encuentra (status 404)
   - Escenario de error interno del servidor (status 500)
   - Escenario de error no encuentra not found (status 204)
...
- **Ubicación obligatoria**: `src/test/java/com/pereira/features/[nombre_microservicio]/`
- **Ejemplo**: `src/test/java/com/pereira/features/tre_msa_savings_account/crearCuentaAhorros.feature`
